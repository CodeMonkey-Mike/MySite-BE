...
const main = async () => {
  try {
    ...
    const SERVED_FOLDER = `/home/dominitech/workspace/${SITE_ORIGIN_DOMAIN}/mikeneder-be/${CIRCLE_PULL_REQUEST}`;
    ...
    const port = Number([PREFIX, CIRCLE_PULL_REQUEST < 10 ? [0, CIRCLE_PULL_REQUEST].join('') : CIRCLE_PULL_REQUEST].join(''));
    ...
    await exec(`echo '${SUDO_PASSWORD}' | sudo -S cp -r ${SERVED_FOLDER}/public /var/www/staging.${SITE_ORIGIN_DOMAIN}/${SITE_URL}`);
    ...
    const vh = `
      ...
      try {
        if(!existsSync(`/etc/nginx/sites-available/api-staging${CIRCLE_PULL_REQUEST}.${SITE_ORIGIN_DOMAIN}`) 
          && !existsSync(`/etc/nginx/sites-enabled/api-staging${CIRCLE_PULL_REQUEST}.${SITE_ORIGIN_DOMAIN}`) ) {
